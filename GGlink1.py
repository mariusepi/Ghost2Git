...
with sqlite3.connect(dbghost_filename) as ghostdb:
 ...
 Edit it after login at %s/ghost/editor/%s
                ...
                sqlquery = "select huuid, hslug, hash, hstatus from hashes where huuid = '%s';" % (uuid)
                hdbcursor.execute(sqlquery)
                for row in hdbcursor.fetchall():
                    ...
                    if  h == hash:
                        ...
                        if hstatus == "draft" and status == "published":
                            ...
                            sqlquery = "UPDATE hashes SET hstatus='%s' WHERE huuid = '%s';" % (status,uuid)
                            hdbcursor.execute(sqlquery)
                            sqlquery = "select email, name from users where id = '%s';" % (author_id)
                            gdbcursor.execute(sqlquery)
                            for row in gdbcursor.fetchall():
 ...
 The editors can review it at %s/ghost/editor/%s
